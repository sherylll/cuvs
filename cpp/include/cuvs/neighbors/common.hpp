...
template <typename CodesMatrixT,
          typename FloatVectorT,
          typename SumVectorT,
          typename CentroidVectorT,
          typename DataT,
          typename IdxT>
struct bbq_dataset_owning_storage {
  using math_type = DataT;

  CodesMatrixT codes;
  FloatVectorT lower_intervals;
  FloatVectorT upper_intervals;
  FloatVectorT additional_corrections;
  SumVectorT quantized_component_sums;
  CentroidVectorT centroid;
  ...
  bbq_dataset_owning_storage(CodesMatrixT&& codes,
                             FloatVectorT&& lower_intervals,
                             FloatVectorT&& upper_intervals,
                             FloatVectorT&& additional_corrections,
                             SumVectorT&& quantized_component_sums,
...
template <typename ContainerType,
          typename CodesMatrixViewT,
          typename FloatVectorViewT,
          typename SumVectorViewT,
          typename CentroidVectorViewT,
          typename DataT,
          typename IdxT,
          typename Accessor>
struct bbq_dataset_view_storage {
  using owning_storage_type = typename ContainerType::
    template owning_storage<DataT, IdxT, dataset_owning_accessor_for_view<DataT, Accessor>>;
  owning_storage_type const* storage_ptr_{nullptr};

  CodesMatrixViewT codes;
  FloatVectorViewT lower_intervals;
  FloatVectorViewT upper_intervals;
  FloatVectorViewT additional_corrections;
  SumVectorViewT quantized_component_sums;
  CentroidVectorViewT centroid;
  ...
  bbq_dataset_view_storage(owning_storage_type const* storage_ptr) noexcept
    : storage_ptr_{storage_ptr},
      codes{storage_ptr_->codes.view()},
      lower_intervals{storage_ptr_->lower_intervals.view()},
      upper_intervals{storage_ptr_->upper_intervals.view()},
      additional_corrections{storage_ptr_->additional_corrections.view()},
      quantized_component_sums{storage_ptr_->quantized_component_sums.view()},
      centroid{storage_ptr_->centroid.view()},
      bits{storage_ptr_->bits},
      layout{storage_ptr_->layout},
      metric{storage_ptr_->metric},
      centroid_norm_sq{storage_ptr_->centroid_norm_sq}
  {
  }
...
struct bbq_dataset_container {
  template <typename DataT, typename IdxT, typename Accessor>
  using owning_storage =
    detail::bbq_dataset_owning_storage<detail::dense_owning_matrix<uint8_t, IdxT, Accessor>,
                                       detail::dense_owning_vector<DataT, IdxT, Accessor>,
                                       detail::dense_owning_vector<int32_t, IdxT, Accessor>,
                                       detail::dense_owning_vector<DataT, IdxT, Accessor>,
                                       DataT,
                                       IdxT>;
  ...
  template <typename DataT, typename IdxT, typename Accessor>
  using view_storage =
    detail::bbq_dataset_view_storage<bbq_dataset_container,
                                     detail::dense_view_matrix<const uint8_t, IdxT, Accessor>,
                                     detail::dense_view_vector<const DataT, IdxT, Accessor>,
                                     detail::dense_view_vector<const int32_t, IdxT, Accessor>,
                                     detail::dense_view_vector<const DataT, IdxT, Accessor>,
                                     DataT,
                                     IdxT,
                                     Accessor>;
...
// -----------------------------------------------------------------------------
// BBQ compressed (view holds non-owning pointer to owning dataset)
// -----------------------------------------------------------------------------

template <typename DataT, typename IdxT, typename Accessor>
struct dataset<bbq_dataset_container, DataT, IdxT, Accessor> {
  using container_type = bbq_dataset_container;
  using owning_storage_type =
    typename container_type::template owning_storage<DataT, IdxT, Accessor>;
  using storage_view_type = typename container_type::
    template view_storage<DataT, IdxT, detail::dataset_view_accessor_for_owning<DataT, Accessor>>;

  dataset(owning_storage_type&& storage) : storage_vector() { add_storage(std::move(storage)); }
  ...
  void add_storage(owning_storage_type&& storage)
  {
    if (storage_vector.size() != 0) {
      RAFT_EXPECTS(storage_vector.front().dim() == storage.dim(),
                   "All storages must have the same dimension");
      RAFT_EXPECTS(storage_vector.front().n_rows() == storage.n_rows(),
                   "All storages must have the same number of rows");
      RAFT_EXPECTS(storage_vector.front().metric == storage.metric,
                   "All storages must have the same metric");
      RAFT_EXPECTS(!has_bit_and_layout(storage.bits, storage.layout),
                   "This bit and layout combination is already in the dataset");
    }
    storage_vector.push_back(std::move(storage));
  }
  std::vector<owning_storage_type> storage_vector;

  [[nodiscard]] auto n_rows() const noexcept
  {
    return storage_vector.size() > 0 ? storage_vector.front().n_rows() : IdxT{0};
  }
  [[nodiscard]] auto dim() const noexcept -> uint32_t
  {
    return storage_vector.size() > 0 ? storage_vector.front().dim() : uint32_t{0};
  }
  bool has_bit_and_layout(uint32_t bits, bbq_code_layout layout) const noexcept
  {
    for (const auto& storage : storage_vector) {
      if (storage.bits == bits && storage.layout == layout) return true;
    }
    return false;
  }
  [[nodiscard]] auto get_storage_view(uint32_t bits, bbq_code_layout layout) const
    -> storage_view_type
  {
    for (size_t i = 0; i < storage_vector.size(); i++) {
      if (storage_vector[i].bits == bits && storage_vector[i].layout == layout)
        return storage_view_type{&storage_vector[i]};
    }
    RAFT_FAIL("This bit and layout combination is not in the dataset");
  }
  auto storage_size() const noexcept { return storage_vector.size(); }
  [[nodiscard]] auto get_front_storage_view() const -> storage_view_type
  {
    RAFT_EXPECTS(storage_vector.size() != 0, "Storage vector is empty");
    return storage_view_type{&storage_vector[0]};
  }
};
...
struct dataset_view<bbq_dataset_container, DataT, IdxT, Accessor> {
  using container_type    = bbq_dataset_container;
  using dataset_type      = dataset<bbq_dataset_container,
                                    DataT,
                                    IdxT,
                                    detail::dataset_owning_accessor_for_view<DataT, Accessor>>;
  using view_storage_type = typename dataset_type::storage_view_type;

  dataset_type const* dataset_ptr_{nullptr};
  dataset_view(dataset_type const* dataset_ptr) : dataset_ptr_{dataset_ptr} {}
  [[nodiscard]] auto n_rows() const noexcept
  {
    return dataset_ptr_ != nullptr ? dataset_ptr_->n_rows() : IdxT{0};
  }
  [[nodiscard]] auto dim() const noexcept -> uint32_t
  {
    return dataset_ptr_ != nullptr ? dataset_ptr_->dim() : uint32_t{0};
  }
  [[nodiscard]] auto dset() const noexcept -> dataset_type const& { return *dataset_ptr_; }
  bool has_bit_and_layout(uint32_t bits, bbq_code_layout layout) const noexcept
  {
    return dataset_ptr_ != nullptr ? dataset_ptr_->has_bit_and_layout(bits, layout) : false;
  }
  [[nodiscard]] auto get_storage_view(uint32_t bits, bbq_code_layout layout) const
    -> view_storage_type
  {
    RAFT_EXPECTS(dataset_ptr_ != nullptr, "Dataset pointer is nullptr");
    return dataset_ptr_->get_storage_view(bits, layout);
  }
  [[nodiscard]] auto get_front_storage_view() const -> view_storage_type
  {
    RAFT_EXPECTS(dataset_ptr_ != nullptr, "Dataset pointer is nullptr");
    return dataset_ptr_->get_front_storage_view();
  }
  auto storage_size() const noexcept
  {
    return dataset_ptr_ != nullptr ? dataset_ptr_->storage_size() : std::size_t{0};
  }
};
...
template <typename IdxT>
using device_bbq_dataset_storage_view =
  dataset_view<bbq_dataset_container, float, IdxT, detail::device_view_accessor<float>>::
    view_storage_type;
...
template <typename DataT, typename IdxT, typename Accessor>
struct is_bbq_dataset<dataset_view<bbq_dataset_container, DataT, IdxT, Accessor>> : std::true_type {
};
